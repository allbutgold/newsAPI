...
function news() {
    const language = document.querySelector('#en').value; // default language value is 'en' for english
    const world = document.querySelector('#input').value; // default input is world
    const category = document.querySelector('#category').value; // default category is 'general'
...
fetch(`${url}${params}`) 
    ...
    .then(data => {
        console.log(data)
            ...
            // article container
            // while (articleContainer.parentElement) {
            //     parentElement.replaceChild(articleContainer.parentElement);
            // }

            articleContainer.appendChild(parentElement);
            // append to the end of the body
            document.querySelector('#newsContent').appendChild(articleContainer);
        })
    })
}
